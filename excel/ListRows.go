package excel

import (
	"github.com/zzl/go-win32api/win32"
	"github.com/zzl/go-com/com"
	"github.com/zzl/go-com/ole"
	"syscall"
	"unsafe"
)

// 00024474-0000-0000-C000-000000000046
var IID_ListRows = syscall.GUID{0x00024474, 0x0000, 0x0000, 
	[8]byte{0xC0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x46}}

type ListRows struct {
	ole.OleClient
}

func NewListRows(pDisp *win32.IDispatch, addRef bool, scoped bool) *ListRows {
	p := &ListRows{ole.OleClient{pDisp}}
	if addRef {
		pDisp.AddRef()
	}
	if scoped {
		com.AddToScope(p)
	}
	return p
}

func ListRowsFromVar(v ole.Variant) *ListRows {
	return NewListRows(v.PdispValVal(), false, false)
}

func (this *ListRows) IID() *syscall.GUID {
	return &IID_ListRows
}

func (this *ListRows) GetIDispatch(addRef bool) *win32.IDispatch {
	if addRef {
		this.AddRef()
	}
	return this.IDispatch
}

func (this *ListRows) QueryInterface_(riid *syscall.GUID, ppvObj unsafe.Pointer)  {
	retVal := this.Call(0x60000000, []interface{}{riid, ppvObj})
	_= retVal
}

func (this *ListRows) AddRef() uint32 {
	retVal := this.Call(0x60000001, nil)
	return retVal.UintValVal()
}

func (this *ListRows) Release() uint32 {
	retVal := this.Call(0x60000002, nil)
	return retVal.UintValVal()
}

func (this *ListRows) GetTypeInfoCount(pctinfo *uint32)  {
	retVal := this.Call(0x60010000, []interface{}{pctinfo})
	_= retVal
}

func (this *ListRows) GetTypeInfo(itinfo uint32, lcid uint32, pptinfo unsafe.Pointer)  {
	retVal := this.Call(0x60010001, []interface{}{itinfo, lcid, pptinfo})
	_= retVal
}

func (this *ListRows) GetIDsOfNames(riid *syscall.GUID, rgszNames **int8, cNames uint32, lcid uint32, rgdispid *int32)  {
	retVal := this.Call(0x60010002, []interface{}{riid, rgszNames, cNames, lcid, rgdispid})
	_= retVal
}

func (this *ListRows) Invoke(dispidMember int32, riid *syscall.GUID, lcid uint32, wFlags uint16, pdispparams *win32.DISPPARAMS, pvarResult *ole.Variant, pexcepinfo *win32.EXCEPINFO, puArgErr *uint32)  {
	retVal := this.Call(0x60010003, []interface{}{dispidMember, riid, lcid, wFlags, pdispparams, pvarResult, pexcepinfo, puArgErr})
	_= retVal
}

func (this *ListRows) Application() *Application {
	retVal := this.PropGet(0x00000094, nil)
	return NewApplication(retVal.PdispValVal(), false, true)
}

func (this *ListRows) Creator() int32 {
	retVal := this.PropGet(0x00000095, nil)
	return retVal.LValVal()
}

func (this *ListRows) Parent() *ole.DispatchClass {
	retVal := this.PropGet(0x00000096, nil)
	return ole.NewDispatchClass(retVal.PdispValVal(), true)
}

var ListRows_Add__OptArgs= []string{
	"Position", 
}

func (this *ListRows) Add_(optArgs ...interface{}) *ListRow {
	optArgs = ole.ProcessOptArgs(ListRows_Add__OptArgs, optArgs)
	retVal := this.Call(0x00000825, nil, optArgs...)
	return NewListRow(retVal.PdispValVal(), false, true)
}

func (this *ListRows) Default_(index interface{}) *ListRow {
	retVal := this.PropGet(0x00000000, []interface{}{index})
	return NewListRow(retVal.PdispValVal(), false, true)
}

func (this *ListRows) NewEnum_() *com.UnknownClass {
	retVal := this.PropGet(-4, nil)
	return com.NewUnknownClass(retVal.PunkValVal(), true)
}

func (this *ListRows) ForEach(action func(item *ListRow) bool) {
	pEnum := this.NewEnum_()
	var pEnumVar *win32.IEnumVARIANT
	pEnum.QueryInterface(&win32.IID_IEnumVARIANT, unsafe.Pointer(&pEnumVar))
	defer pEnumVar.Release();
	for {
		var c uint32
		var v ole.Variant
		pEnumVar.Next(1, (*win32.VARIANT)(&v), &c)
		if c == 0 {
			break
		}
		pItem := (*ListRow)(v.ToPointer())
		ret := action(pItem)
		v.Clear()
		if !ret {
			break
		}
	}
}

func (this *ListRows) Item(index interface{}) *ListRow {
	retVal := this.PropGet(0x000000aa, []interface{}{index})
	return NewListRow(retVal.PdispValVal(), false, true)
}

func (this *ListRows) Count() int32 {
	retVal := this.PropGet(0x00000076, nil)
	return retVal.LValVal()
}

var ListRows_Add_OptArgs= []string{
	"Position", "AlwaysInsert", 
}

func (this *ListRows) Add(optArgs ...interface{}) *ListRow {
	optArgs = ole.ProcessOptArgs(ListRows_Add_OptArgs, optArgs)
	retVal := this.Call(0x000000b5, nil, optArgs...)
	return NewListRow(retVal.PdispValVal(), false, true)
}

